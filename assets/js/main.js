...
(function ($) {
    ...
    var deleteFile = function (e) {
        ...
        function updateTotalPrice() {
            ...
            $(".list-file-delete .tf-mini-cart-item").each(function () {
                var priceText = $(this).find(".tf-mini-card-price").text().replace("KES ", "").replace(",", "").trim();
            ...
            var formatted = total.toLocaleString("en-US", { style: "currency", currency: "KES" });
            $(".tf-totals-total-value").text(formatted);
        }

        function updatePriceEach() {
            $(".each-prd").each(function () {
                var priceText = $(this).find(".each-price").text().replace("KES ", "").replace(",", "").trim();
                var price = parseFloat(priceText);
                var quantity = parseInt($(this).find(".quantity-product").val(), 10);
                if (!isNaN(price) && !isNaN(quantity)) {
                    var subtotal = price * quantity;
                    var formatted = subtotal.toLocaleString("en-KE", { style: "currency", currency: "KES" });
        ...
        function updateTotalPriceEach() {
            ...
            var formatted = total.toLocaleString("en-KE", {
                style: "currency",
                currency: "KES"
            });
    ...
    var totalPriceVariant = function () {
        $(".tf-product-info-wrap").each(function () {
            ...
            if (!priceEl.data("price")) {
                var initialPrice = parseFloat(priceEl.text().replace("KES ", "").replace(/,/g, ""));
    ...
    var popupProductVariant = () => {
        ...
        $(".tf-quick-prd_variant").each(function () {
            ...
            // $wrap.find(".price-add").text("KES " + basePrice.toFixed(0));
            $wrap.find(".price-add").text("KES " + Number(basePrice).toLocaleString('en-KE', { maximumFractionDigits: 0 }));
            ...
            $wrap.find(".size_btn:not(.disabled)").on("click", function () {
                ...
                //$wrap.find(".price-on-sale").text("KES " + price.toFixed(0));
                $wrap.find(".price-on-sale").text("KES " + Number(price).toLocaleString('en-KE', { maximumFractionDigits: 0 }));
            ...
            function updateAddPrice() {
                ...
                // $wrap.find(".price-add").text("KES " + total.toFixed(0));
                $wrap.find(".price-add").text("KES " + Number(total).toLocaleString('en-KE', { maximumFractionDigits: 0 }));
            }
        });
    };
